...
errHandler:
    ...
    sfsub错误处理 "体检界面部件", "frmSelectItem", "ccmdOk_Click", 6666, lstrError, False
...
'    Dim i As Long
'    Dim lstrItem As String
'    Set lobjRec = CreateObject("职业病史录入.clsmedicalprojt")
'    If Node.Parent Is Nothing Then
'        '当前操作的是父节点。
'        '自动选中(或不选中)所有子节点。
'        If Node.Children > 0 Then
'            lstrItem = Right(Node.Key, Len(Node.Key) - 1)
'            If Node.Checked = False Then
'                lobjRec.subDelParent (lstrItem)
'            Else
'                For i = 1 To Node.Children
'                    Set lobjNode = ctrwItem.Nodes(Node.Index + i)
'                    lstrItem = Right(Node.Key, Len(Node.Key) - 1)
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "体检界面部件", "frmSelectItem", "Form_Load", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
