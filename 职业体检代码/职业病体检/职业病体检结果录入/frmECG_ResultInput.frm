...
Begin VB.Form frmECG_ResultInput 
   Caption         =   "心电科结果录入窗口"
   ClientHeight    =   10275
   ClientLeft      =   60
   ClientTop       =   345
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin TabDlg.SSTab ctabPerson 
            ...
            EndProperty
            TabCaption(0)   =   "   单个处理  "
            TabPicture(0)   =   "frmECG_ResultInput.frx":0000
            ...
            TabCaption(1)   =   "   批量处理 "
            TabPicture(1)   =   "frmECG_ResultInput.frx":001C
   ...
   End
End
Attribute VB_Name = "frmECG_ResultInput"
...
'    On Error Resume Next
    ...
    If ccmbHistory.Text <> "——" Then
        ...
        Set lobjRec = mobj体检医师.func获取指定年份的体检病历(Trim(ctxtSingleNo.Text), ccmbHistory.Text, "心电科")
...
'            cgrdInput.Height = cgrdInput.Height - 300
            ...
            Set lobjRec = mobj体检医师.func获取指定年份的体检病历结论(Trim(ctxtSingleNo.Text), "10", Trim(ccmbHistory.Text))
            If Not lobjRec Is Nothing Then
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "ccmdSingleQuery_Click", 6666, lstrError, False
End Sub
''''功能：改变选择外观
''''作者：翁乔
''''时间：2012-06-01
...
Private Sub Cmd结论模板_Click()
    frmConclusion.lobj科室 = "心电科"
    frmConclusion.lobj科室编号 = "10"
...
'''    coptClasses(2).Enabled = False
    ...
    If lobjTmp.func获取体检人员体检科室信息(lstrNo, priDeptName) Then
        ...
        If cgrdInfoBatch.rows > 1 Then ctbMain.Buttons(1).Enabled = True
...
'''    ctxtSingleNo.Text = mstr系统编号固定部分
    ...
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_心电科结果录入_修改") = False Then
        ctbMain.Buttons(2).Visible = False
    End If
    
    '2012-05-22 翁乔 ↓↓↓
    '界面权限设置
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_心电科结果录入_批量修改") = False Then
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "Form_Load", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "cgrdInput_BeforeEdit", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "ctxtSingleNo_KeyDown", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
'            End If
    ...
    Exit Sub
errHandler:
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "subShowInputGrid", Err.Number, Err.Description, True
    Exit Sub
    Resume
End Sub
...
Private Sub subShowSinglePerson()
    ...
    If lstrNo <> "" Then
        ...
        If ctabPerson.Tab = 0 Then
            ...
            Set lobjDatecobo = mobj体检医师.func获取体检人员的体检病历(Trim(ctxtSingleNo.Text), "心电科")
...
errHandler:
    If ctabPerson.Tab = 0 Then
        ctbMain.Buttons(1).Enabled = False
    End If
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "subShowSinglePerson", Err.Number, Err.Description, True
    Exit Sub
    Resume
End Sub
...
'            Next
    ...
    Case "退出"
        ...
        End If
        Unload Me
        Set frmECG_ResultInput = Nothing
    '2012-06-21 于登淼 ↑
    End Select
    Exit Sub
...
Private Sub subSaveBatch()
    ...
    For i = 1 To barCode.Count
        isOk = lobjTmp.func保存单人体检结果(barCode(i), um用户名, DTP录入日期.Value, lcolItem, lcolResult, "职业病体检_结果信息_心电科")
