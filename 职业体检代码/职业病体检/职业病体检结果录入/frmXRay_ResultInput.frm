...
Begin VB.Form frmXRay_ResultInput 
   Caption         =   "X光影像科结果录入窗口"
   ClientHeight    =   9780
   ClientLeft      =   60
   ClientTop       =   450
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin TabDlg.SSTab SSTPersonalInfo 
            ...
            ForeColor       =   8388608
            TabCaption(0)   =   "单个录入"
            TabPicture(0)   =   "frmXRay_ResultInput.frx":0000
            ...
            TabCaption(1)   =   "批量录入"
            TabPicture(1)   =   "frmXRay_ResultInput.frx":001C
            ...
            Tab(1).Control(6)=   "Timerccrp"
            Tab(1).Control(6).Enabled=   0   'False
            Tab(1).Control(7)=   "ccmdSelInfo"
            Tab(1).Control(7).Enabled=   0   'False
            Tab(1).Control(8)=   "cchkCompanyBatch"
            Tab(1).Control(8).Enabled=   0   'False
            Tab(1).Control(9)=   "ctxtQueyCompanyBatch"
            Tab(1).Control(9).Enabled=   0   'False
            Tab(1).Control(10)=   "ccmd查询单位"
   ...
   End
End
Attribute VB_Name = "frmXRay_ResultInput"
...
errHandler:
    ...
    sfsub错误处理 "职业病体检结果录入", "frmXRay_ResultInput", "ccmdQuery_Click", 6666, lstrError, False
End Sub
'2012-07-13 于登淼
...
Private Sub Form_Load()
    ...
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_X光影像科结果录入_修改") = False Then
        ctlb工具栏.Buttons(2).Visible = False
    End If
    
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_X光影像科结果录入_删除") = False Then
        ctlb工具栏.Buttons(4).Visible = False
    End If
    
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_X光影像科结果录入_网络配置") = False Then
        ctlb工具栏.Buttons(5).Visible = False
    End If
    
    '2012-05-22 翁乔 ↓↓↓
    '界面权限设置
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_X光影像科结果录入_批量修改") = False Then
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "frmXRay_ResultInput", "Form_Load", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
'            End If
        ...
        End If
        '2012-06-21 于登淼 ↑
        Unload frmXRay_ResultInput
        Set frmXRay_ResultInput = Nothing
    End Select
    
    Exit Sub
errHandler:
    If Err.Number = 0 Then Exit Sub
    sfsub错误处理 "职业病体检结果录入", "frmXRay_ResultInput", "mobjGUI_BeforeOperate", Err.Number, Err.Description, False
    Cancel = True
    Exit Sub
    Resume
End Sub
...
'''        If mlobjRec("体检类型") = "涉核部队" Then coptClasses(2).Value = True
        ...
        '显示病人的历年病历。（翁乔；2012-07-31）↓↓↓↓↓↓↓↓↓↓↓↓
            ...
            Set lobjDatecobo = mobj体检医师.func获取体检人员的体检病历(Trim(ctxtBarCode.Text), "X光影像科")
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "frmXRay_ResultInput", "LoadPersonalInfo", 6666, lstrError, False
End Sub
...
Sub subSaveBatch(ByVal para系统编号 As String)
    ...
    Dim lobjTmp As Object
    Set lobjTmp = CreateObject("职业病体检结果录入.clscommon")
    isOk = lobjTmp.func保存单人体检结果(para系统编号, mstrDoctorName, cdtpConclusionDate.Value, lcolItem, lcolResult, "职业病体检_结果信息_X光影像科")
...
Sub subSave()
    ...
    Dim lobjTmp As Object
    Set lobjTmp = CreateObject("职业病体检结果录入.clscommon")
    isOk = lobjTmp.func保存单人体检结果(ctxtBarCode.Text, mstrDoctorName, cdtpConclusionDate.Value, lcolItem, lcolResult, "职业病体检_结果信息_X光影像科")
    subClear
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "frmXRay_ResultInput", "subSave", 6666, lstrError, False
End Sub
