...
Begin VB.Form FrmInMedi_ResultInput 
   Caption         =   "内科结果录入窗口"
   ClientHeight    =   10200
   ClientLeft      =   60
   ClientTop       =   345
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin TabDlg.SSTab ctabPerson 
            ...
            EndProperty
            TabCaption(0)   =   "   单个处理  "
            TabPicture(0)   =   "frmInMedi_ResultInput.frx":0000
            ...
            TabCaption(1)   =   "   批量处理 "
            TabPicture(1)   =   "frmInMedi_ResultInput.frx":001C
   ...
   End
End
Attribute VB_Name = "FrmInMedi_ResultInput"
...
'    On Error Resume Next
    ...
    If ccmbHistory.Text <> "——" Then
        ...
        Set lobjRec = mobj体检医师.func获取指定年份的体检病历(Trim(ctxtSingleNo.Text), ccmbHistory.Text, "内科")
...
'            cgrdInput.Height = cgrdInput.Height - 300
            ...
            Set lobjRec = mobj体检医师.func获取指定年份的体检病历结论(Trim(ctxtSingleNo.Text), "02", Trim(ccmbHistory.Text))
            If Not lobjRec Is Nothing Then
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "ccmdSingleQuery_Click", 6666, lstrError, False
End Sub
...
'''    ctxtSingleNo.Text = mstr系统编号固定部分
    ...
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_内科结果录入_修改") = False Then
        ctbMain.Buttons(2).Visible = False
    End If
    
    '2012-05-22 翁乔 ↓↓↓
    '界面权限设置
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_内科结果录入_批量修改") = False Then
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "Form_Load", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "cgrdInput_BeforeEdit", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "ctxtSingleNo_KeyDown", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
'            End If
    ...
    Exit Sub
errHandler:
    sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "subShowInputGrid", Err.Number, Err.Description, True
    Exit Sub
    Resume
End Sub
...
Private Sub subShowSinglePerson()
    ...
    If lstrNo <> "" Then
        ...
        If ctabPerson.Tab = 0 Then
            ...
            Set lobjDatecobo = mobj体检医师.func获取体检人员的体检病历(Trim(ctxtSingleNo.Text), "内科")
...
'    Exit Sub
'errHandler:
'    sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "subShowBatchPerson", Err.Number, Err.Description, True
'    mblnSys = False
'    Exit Sub
'    Resume
'End Sub
...
Private Sub mobj界面通用对象_BeforeOperate(ByVal Operate As String, Cancel As Boolean)
    ...
    Case "退出"
        ...
        End If
        Unload Me
        Set FrmInMedi_ResultInput = Nothing
    '2012-06-21 于登淼 ↑
    End Select
    Exit Sub
...
errHandler:
    If Err.Number <> 0 Then
        Dim lstrError As String
        lstrError = func错误处理(Err.Number, Err.Description)
        sfsub错误处理 "职业病体检结果录入", "FrmInMedi_ResultInput", "mobj界面通用对象_BeforeOperate", 6666, lstrError, False
    End If
    If Operate = "保存" Then
        '恢复界面可以操作。
        ctbMain.Enabled = True
...
Private Sub subSaveBatch()
    ...
    For i = 1 To barCode.Count
        isOk = lobjTmp.func保存单人体检结果(barCode(i), um用户名, DTP录入日期.Value, lcolItem, lcolResult, "职业病体检_结果信息_内科")
