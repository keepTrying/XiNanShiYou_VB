...
Public Property Get pblnInUse() As Boolean
    ...
    Exit Property
errHandler:
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "Property Get pblnInUse", Err.Number, Err.Description, True
End Property
...
errHandler:
    ...
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "Form_Load", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
'    End If
    ...
    If ctbMain.Buttons(5).Enabled Then
        '要取消结论，获取已下结论的体检记录。
        Set lobjRec = pobj业务对象.Func获取体检结论已确定的体检记录(IIf(coptBatch(0).Value, Format(cdtpStart.Value, "yyyy-mm-dd hh:mm:ss"), ""), "", IIf(ccmbSheet.ListIndex > 0, ccmbSheet.Text, ""), IIf(coptBatch(2).Value, ctxtNo.Text, ""))
    Else
        '要保存结论，获取还未下结论的体检记录。
        If Not coptBatch(1).Value Then
            Set lobjRec = pobj业务对象.Func获取已下结论但未确定的体检记录(IIf(coptBatch(0).Value, Format(cdtpStart.Value, "yyyy-mm-dd hh:mm:ss"), ""), "", IIf(ccmbSheet.ListIndex > 0, ccmbSheet.Text, ""), IIf(coptBatch(2).Value, ctxtNo.Text, ""))
        Else
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "ccmdAddPerson_Click", 6666, lstrError, False
    cgrdPerson.Redraw = True
    Exit Sub
    Resume
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "ccmdUpdateConclusion_Click", 6666, lstrError, False
    Exit Sub
    Resume
End Sub
...
'    csbMain.Panels(1) = "正在获取当前体检人员的体检结果，请稍候..."
    ...
    '创建体检表对象。
    Set lobj体检表 = CreateObject("职业病对象.clsMedicalExamSheet")
    ...
    If ctbMain.Buttons(5).Enabled Then
        If IsDate(cgrdPerson.TextMatrix(cgrdPerson.Row, mcolFieldIndex("下结论日期"))) Then
            cdtpDate.Value = Format(cgrdPerson.TextMatrix(cgrdPerson.Row, mcolFieldIndex("下结论日期")), "yyyy-mm-dd hh:mm:ss")
        Else
            cdtpDate.Value = Format(Now, "yyyy-mm-dd hh:mm:ss")
        End If
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "cgrdPerson_Click", 6666, lstrError, False
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "subReset", 6666, lstrError, True
End Sub
...
'            csbMain.Panels(1) = "正在确定第" & i & "个人（共" & (llngRow) & "个人）的体检结论。 "
            ...
            With cgrdPerson
                ...
                Else
                    lbln允许办证 = False
                End If
                pobj业务对象.Sub确定体检结论 .TextMatrix(1, mcolFieldIndex("系统编号")), .TextMatrix(1, mcolFieldIndex("体检结论")), Format(cdtpDate.Value, "yyyy-mm-dd hh:mm:ss"), .TextMatrix(1, mcolFieldIndex("诊断和处理意见")), .TextMatrix(1, mcolFieldIndex("复查体检表名")), lstr文书名称, lbln是否预览文书, lbln允许办证
            End With
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "frmGiveConclusion", "ctbMain_ButtonClick", 6666, lstrError, False
    MousePointer = 0
'    csbMain.Panels(1) = ""
    Exit Sub
    Resume
