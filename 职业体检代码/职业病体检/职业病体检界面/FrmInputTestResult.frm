...
Begin VB.Form FrmInputTestResult 
   ...
   Begin TabDlg.SSTab ctabPerson 
      ...
      Tab(1).Control(0)=   "coptBatchType(2)"
      Tab(1).Control(1)=   "coptBatchType(1)"
      Tab(1).Control(2)=   "coptBatchType(0)"
      Tab(1).Control(3)=   "ctxtBatchNo"
      Tab(1).Control(4)=   "cchkEnd(1)"
      Tab(1).Control(5)=   "cchkUnEnd(1)"
      Tab(1).Control(6)=   "ctxt单位名称"
      Tab(1).Control(7)=   "ccmbSheet"
      Tab(1).Control(8)=   "ccmdClear"
      Tab(1).Control(9)=   "ccmdRemove"
      Tab(1).Control(10)=   "ccmdAdd"
      Tab(1).Control(11)=   "cpicPhoto(1)"
      Tab(1).Control(12)=   "cgrdPerson"
      Tab(1).Control(13)=   "cdtpQueryDate"
...
errHandler:
    If Err <> 0 Then
        Dim lstrError As String
        lstrError = func错误处理(Err.Number, Err.Description)
        sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "cgrdPerson_Click", 6666, lstrError, False
    End If
    Set lobj体检 = Nothing
    '界面恢复可以操作。
...
errHandler:
    If Err <> 0 Then
        Dim lstrError As String
        lstrError = func错误处理(Err.Number, Err.Description)
        sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "cgrdSingleList_SelChange", 6666, lstrError, False
    End If
    '界面恢复可以操作。
    ctbMain.Enabled = True
    ctabPerson.Enabled = True
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "ccmdAdd_Click", 6666, lstrError, False
    cdtpQueryDate.SetFocus
    cstbMain.Panels(1) = ""
    MousePointer = 0
    Exit Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "cdtpQueryDate_KeyDown", 6666, lstrError, False
End Sub
...
errHandler:
    mblnSys = False
    sfsub错误处理 "职业病界面部件", "FrmInputTestResult", "subShowSingleList", Err.Number, Err.Description, True
    Exit Sub
    Resume
End Sub
