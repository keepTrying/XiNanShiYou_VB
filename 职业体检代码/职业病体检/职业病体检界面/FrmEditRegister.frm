...
Private mobj体检 As Object                   '职业病对象
...
Public mblnInUse As Boolean                 '对应属性pblnInUse。
...
Private Sub cdtpDate_GotFocus()
    If DateDiff("d", cdtpDate.Value, Date) > 0 Then
        MsgBox "体检日期不对！"
        cdtpDate.Value = Date
    End If
End Sub
...
Private Sub ctxtAge_GotFocus()
    ...
    If IsNumeric(ctxtAge.Text) Then
    With ctxtAge
        .SelStart = 0
...
errHandler:
    ...
    sfsub错误处理 "职业病界面部件", "FrmEditRegister", "Form_Load", 6666, lstrError, False
...
errHandler:
    If Err <> 0 Then
        Dim lstrError As String
        lstrError = func错误处理(Err.Number, Err.Description)
        sfsub错误处理 "职业病界面部件", "FrmEditRegister", "Timer1_Timer", 6666, lstrError, False
    End If
...
Private Sub ccmbTemplate_Click()
    ...
    Set lobj体检表模板 = CreateObject("职业病对象.clsMedicalExamTemplate")
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmEditRegister", "ccmbTemplate_Click", 6666, lstrError, False
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmEditRegister", "ccmdLocateUnit_Click", 6666, lstrError, False
End Sub
...
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmEditRegister", "ctxtSysNo_LostFocus", 6666, lstrError, False
...
'                .体检人员.像片压缩 = cctlCatchPhoto.Photo
            ...
            End If
            .体检日期 = Format(cdtpDate.Value, "yyyy-mm-dd hh:mm:ss")
...
errHandler:
    ...
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmRegisterFirst", "mobjGUI_BeforeOperate", 6666, lstrError, False
...
errHandler:
    ...
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
    sfsub错误处理 "职业病界面部件", "FrmRegisterFirst", "mobjGUI_ItemLostFocus", 6666, lstrError, False
...
Private Sub subShowRegisterInfo()
    ...
    Exit Sub
errHandler:
    sfsub错误处理 "职业病界面部件", "FrmEditRegister", "subShowRegisterInfo", Err.Number, Err.Description, True
    Exit Sub
    Resume
End Sub
