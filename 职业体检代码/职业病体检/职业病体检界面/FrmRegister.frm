...
Begin VB.Form FrmRegister 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Tab(0).Control(18)=   "ccmb体检类别"
      Tab(0).Control(18).Enabled=   0   'False
      Tab(0).Control(19)=   "ccmb体检类型"
      ...
      Tab(0).Control(23)=   "ctxt身份证号"
      Tab(0).Control(23).Enabled=   0   'False
      Tab(0).Control(24)=   "clblsysno"
      Tab(0).Control(24).Enabled=   0   'False
      Tab(0).Control(25)=   "frmPhoto"
      Tab(0).Control(25).Enabled=   0   'False
      Tab(0).Control(26)=   "cdtp出生"
      Tab(0).Control(26).Enabled=   0   'False
      Tab(0).Control(27)=   "ccmb体检人类型"
      Tab(0).Control(27).Enabled=   0   'False
      Tab(0).Control(28)=   "Ccmb体检人类别"
...
Private Sub ccmb体检人类型_Click()
    ...
    '2015-3-13刘伟 取消根据不同体检人员类型，生成不同的系统编号，保持编号不变
...
'modify by lanchao 2015-03-12 if else 注释取消
        Ccmb体检人类别.Text = "在岗期间"
    Else
...
Private Sub cgrdHistory_DblClick()
    ...
    clblsysno_LostFocus
    clblsysno.Text = lstrSysNo        '只是抓取以前的信息并且重新编号？
End Sub
...
'    ctbMain.Buttons(5).Visible = False
    ...
    If lobjTmp.func科室操作权限(um用户编号, "职业病体检_体检登记_初检登记_校核通过") = False Then
        ctbMain.Buttons(7).Visible = False
    End If
    '2012-06-15 于登淼 ↑
...
'''    '2012-08-19 于登淼 ↑
    ...
    Timer2.Enabled = True  '2016-2-24 by 牟俊
    Exit Sub
errHandler:
    Dim lstrError As String
    lstrError = func错误处理(Err.Number, Err.Description)
...
Private Sub subClear()
    ...
    On Error Resume Next
    '录入界面中所有控件恢复formload
'    ccmb体检人类型.Text = ccmb体检人类型.List(0)
'    Ccmb体检人类别.Text = Ccmb体检人类别.List(0)
    '2012-06-14 于登淼 ↓
    '体检人类型与类别的text赋值后，必须将ListIndex同时调整
'    ccmb体检人类型.ListIndex = 0
'    Ccmb体检人类别.ListIndex = 0
    '2012-06-14 于登淼 ↑
    ...
    ctxt邮编.Text = ""
    ctxt住址.Text = ""
    ccmb文化程度.Text = ccmb文化程度.List(0)
    Ccmb婚否.Text = Ccmb婚否.List(0)
    ccmb民族.Text = ccmb民族.List(0)
    ctxt电话.Text = ""
    ctxt工龄.Text = ""
    ctxt出生地.Text = ""
    ccmb照射源.Text = ccmb照射源.List(0)
    ccmb职业类别.Text = ccmb照射源.List(0)
    ccmb危害因素.Text = ccmb危害因素.List(0)
    ccmb现工种.Text = ccmb现工种.List(0)
    ccmb职务.Text = ccmb职务.List(0)
    ctxt危害工龄.Text = ""
    ctxt放射剂量.Text = ""
...
Private Sub mobjGUI_BeforeOperate(ByVal Operate As String, Cancel As Boolean)
    ...
    Case "保存"
        ...
        frmProcess.Label1.Caption = "正在保存，请等待..."
        frmProcess.proPercent.Value = 1
        frmProcess.Show 0, Me
        DoEvents
        '2012-08-18 于登淼 ↓
        '复查保存有诸多信息可以省略判断，故省去判断，另外保存。
        '且复查可能使用新的系统编号，故必须将原来与复查信息分开保存。
        If 访问标志 = 2 Then
        ...
        If dafuncGetData("select 体检日期 from 职业病体检_体检基本数据库 where 公民身份号码 = '" & Trim(ctxt身份证号.Text) & "' and convert(varchar(10),体检日期,111) = convert(varchar(10),getdate(),111)  and 体检状态 <> 0 ").RecordCount > 0 Then
            ...
            '2012-06-15 于登淼 ↓
             ...
             If mintState = 0 Then
                 ...
                 func身份证认证
                 mblnTakePhoto = False
                 pobj业务对象.func写入单人当前体检状态 clblsysno.Text, mintState
            ...
            访问标志 = 0
             ...
             ctxt籍贯.SetFocus
             'modify by lanchao 2015-03-12 取消以下2行注释
             subClear
             frmRegisterManage.sub查询并显示
             Unload frmProcess
             Cancel = True
...
'        End If
        ...
        'lobj体检表编号.体检表编号 = ccmbTemplate.ItemData(ccmbTemplate.ListIndex(1))
        lobj体检表编号.体检表编号 = ccmbTemplate.Text
        'Set lobjrec类型 = CreateObject("职业病对象.clsmedicalexam")
        'lobjrec1.体检类型 = ccmb体检人类型.ItemData(ccmb体检人类型.ListIndex)
        'lobjrec1.体检类别 = ccmb体检人类别.ItemData(ccmb体检人类别.ListIndex)
...
'                End If
           ...
           ' Next i
            ...
            '.体检人类型 = ccmb体检人类型.ItemData(ccmb体检人类型.ListIndex)
            '.体检人类别 = Ccmb体检人类别.ItemData(Ccmb体检人类别.ListIndex)
      ...
      If mcol收费项目.Count > 0 Then
        ...
        lobjRec.mstr系统编号 = Trim(clblsysno.Text)
...
'        Exit Sub   '2016-2-24 by 牟俊
    ...
    Case "校核通过"
        ...
        End If
        If Trim(ccmbSex.Text) = "" Then
            MsgBox "姓别不能为空！", vbExclamation, "系统提示"
            Exit Sub
        End If
        If Trim(ctxtAge.Text) = "" Then
            MsgBox "年龄不能为空！", vbExclamation, "系统提示"
        ...
        '2012-12-18 刘云乐
            ...
            If mblnTakePhoto Then
                ...
                End If
            End If
            If 访问标志 = 2 And pstr复查系统编号 <> "" Then
                pobj业务对象.func写入单人当前体检状态 pstr复查系统编号, mintState
                pobj业务对象.func写入校核人信息 pstr复查系统编号, um用户编号
            Else
                pobj业务对象.func写入单人当前体检状态 clblsysno.Text, mintState
                pobj业务对象.func写入校核人信息 clblsysno.Text, um用户编号
            End If
    ...
    Case "退出"
        ...
        Set lobjRec系统编号退回 = dafuncGetData("select * from 职业病体检_体检基本数据库 where left(系统编号,len(系统编号)-1)='" & paraSysNo & "'")
        '2012-07-11 于登淼 ↓
        '退出时，询问信息是否已保存（写得较简单）
        i = -vbYes
        If Len(ctxt身份证号.Text) > 0 And Len(ctxtName.Text) > 0 And mintState <> 2 Then i = MsgBox("保存当前信息吗？", vbYesNo) 'i = vbYes
        If i = vbYes Then
'            mintState = 0
            mobjGUI_BeforeOperate "保存", True
        Else
        '2012-07-11 于登淼 ↑
            If lobjRec系统编号退回.RecordCount = 0 Then
                Set lobjRec系统编号退回 = CreateObject("职业病对象.clsMedicalExam")
                lobjRec系统编号退回.Func退回职业病体检系统编号 paraSysNo
            End If
        End If
        Set lobjRec系统编号退回 = Nothing
        Unload Me
        '2012-06-14 于登淼 ↑
    End Select
    Exit Sub   '2016-2-24 by 牟俊
...
'    Set lobjRec系统编号退回 = dafuncGetData("select * from 职业病体检_体检基本数据库 where left(系统编号,len(系统编号)-1)='" & paraSysNo & "'")
'    i = -vbYes
'    If Len(ctxt身份证号.Text) > 0 And Len(ctxtName.Text) > 0 And mintState <> 2 Then i = vbYes 'i = MsgBox("保存当前结果吗？", vbYesNo)
'    If i = vbYes Then
'        mobjGUI_BeforeOperate "保存", True
'    Else
'
...
'获取照射源字典
Private Sub func照射源()
    Dim lobj照射源 As Object
    Dim lcol照射源 As Object
    Dim i As Integer
    On Error GoTo errHandler
    ccmb照射源.Visible = True
    Set lobj照射源 = CreateObject("职业病对象.clsmedicalexamtemplateset")
    Set lcol照射源 = lobj照射源.照射源
    ccmb照射源.Clear
    ccmb照射源.AddItem ""
    For i = 1 To lcol照射源.RecordCount
        ccmb照射源.AddItem lcol照射源("名称")
        ccmb照射源.ItemData(ccmb照射源.NewIndex) = lcol照射源("编号")
        lcol照射源.MoveNext
    Next
    ccmb照射源.Text = ccmb照射源.List(0)
    Set lobj照射源 = Nothing
    ...
    Exit Sub
errHandler:
    sfsub错误处理 "职业病对象部件", "clsMedicalExam", "sub获取文化程度字典", Err.Number, Err.Description, True
End Sub

'获取职业类别字典
Private Sub func职业类别()
    Dim lobj职业类别 As Object
    Dim lcol职业类别 As Object
    Dim i As Integer
    On Error GoTo errHandler
    ccmb职业类别.Visible = True
    Set lobj职业类别 = CreateObject("职业病对象.clsmedicalexamtemplateset")
    lobj职业类别.int照射源 = ccmb照射源.ItemData(ccmb照射源.ListIndex)
    Set lcol职业类别 = lobj职业类别.职业类别
    ccmb职业类别.Clear
    ccmb职业类别.AddItem ""
    For i = 1 To lcol职业类别.RecordCount
        ccmb职业类别.AddItem lcol职业类别("名称")
        ccmb职业类别.ItemData(ccmb职业类别.NewIndex) = lcol职业类别("编号")
        lcol职业类别.MoveNext
    Next
    ccmb职业类别.Text = ccmb职业类别.List(0)
    Set lobj职业类别 = Nothing
    Set lcol职业类别 = Nothing
    Exit Sub
errHandler:
    sfsub错误处理 "职业病对象部件", "clsMedicalExam", "sub获取文化程度字典", Err.Number, Err.Description, True
End Sub
...
Private Sub func获取行业类别字典()
    ...
    Exit Sub
errHandler:
    sfsub错误处理 "职业病对象部件", "clsMedicalExam", "sub func获取行业类别字典", Err.Number, Err.Description, False
End Sub
...
Private Function func获取单位信息(单位编号 As String)
    ...
    If Not lobj单位.RecordCount = 0 Then
        ...
        ccmb经济性质.Text = IIf(IsNull(lobj单位("经济性质")), "", lobj单位("经济性质"))
        Ccmb行业类别.Text = IIf(IsNull(lobj单位("行业类别")), "", lobj单位("行业类别"))
...
'将timer2时间改为900ms  2016-1-6 by 牟俊
...
''    从查询页面传一个参数，代表是查询页面跳转过来的。不能只判断ctxt身份证号.Text
'     If (ctxt身份证号.Text <> "") Then
'    '在这里查询体检人员基本信息表里的体检类型和体检类别以及危害因素。组成的结构如“职业体检-在岗期间-粉尘”modify by liuwei
'
'
...
'
'
'    For ccmbi = 0 To ccmb体检人类型.ListCount - 1
'    If ccmb体检人类型.List(ccmbi) = s体检表类型 Then
'    ' ccmb体检人类型.Text = ccmb体检人类型.List(ccmbi)
'     ccmb体检人类型.ListIndex = ccmb体检人类型.ItemData(ccmbi) - 1
'
'    End If
'    Next ccmbi
'    For ccmbj = 0 To Ccmb体检人类别.ListCount - 1
'    If Ccmb体检人类别.List(ccmbj) = s体检表类别 Then
'    ' Ccmb体检人类别.Text = Ccmb体检人类别.List(ccmbj)
'     Ccmb体检人类别.ListIndex = Ccmb体检人类别.ItemData(ccmbj) - 1
'
'    End If
'    Next ccmbj
'
'    '这个地方有问题，需要修改 lable by lanchao 2015-03-15
'    For ccmbk = 0 To ccmbTemplate.ListCount - 1
'     If ccmbTemplate.List(ccmbk) = s体检表信息 Then
...
'Else
    ...
    If (ret) Then
       ...
       '不做判断，直接读取身份证信息  modify by lanchao 2015-9-10
...
'        sub查看历史信息 (Trim(ctxt身份证号.Text))    '上面已经查询过历史信息了，此处屏蔽  2016-1-15 by 牟俊
'        '2012-07-15 于登淼 ↑
...
For ccmbk = 0 To ccmbTemplate.ListCount - 1
  ...
  End If
Next ccmbk
ccmbTemplate.Text = s体检表信息     '2015-12-10 by 牟俊
End If
'End If
...
'    strSQL = "select 系统编号,建档日期 建档时间,体检表编号 体检表,体检状态 from 职业病体检_体检基本数据库 where 公民身份号码='" & paraIDCard & "' and 建档日期>='" & Format(DateAdd("yyyy", -5, Now), "yyyy-mm-dd") & "'"
    ...
    If lobjRec.RecordCount > 0 Then
        ...
        initState(4) = "未下结论"
'        initState(5) = "已下结论"
        initState(5) = "待复核"    '2015-12-15 by 牟俊
        initState(6) = "已复核"
        initState(7) = "已发报告"
        initState(8) = "待复查"
