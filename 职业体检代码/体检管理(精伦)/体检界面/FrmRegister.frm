...
'                End If
    ...
    Case "修改"
        ...
        '获取当天最后的号。
        If Val(Right(clblSysNo.Caption, Len(clblSysNo) - Len(mobj体检.系统编号固定部分))) > 1 Then
            FrmEditRegister.系统编号 = mobj体检.系统编号固定部分 & Format(Val(Right(clblSysNo.Caption, Len(clblSysNo) - Len(mobj体检.系统编号固定部分))) - 1, String(Len(clblSysNo) - Len(mobj体检.系统编号固定部分), "0"))
        Else
...
'            On Error Resume Next
'             '读取身份证号
'            ciptBase.Box1("身份证号").Text = StrConv(uIDCardNo, vbUnicode)
'
'            '读取图片
...
'        ret = MsgBox("找到卡，正在读卡...", vbOKOnly + vbInformation, "提示")
       ...
       ret = ReadBaseInfos(iname, isex, folk, birthday, code, addr, agency, startdate, enddate)
         'ret = ReadBaseMsgW(Msg, LenT)
         'ret = ReadBaseMsg(Msg1, LenT)
        'ret = GetSAMIDToStr(SAMID)
        If (ret) Then
            'StateLabel.Caption = "读卡成功！"
            
            strMsg = "姓名：" + Trim(iname)
            '读取姓名
            ctxtName.Text = Trim(iname)
            ctxtName.Text = Trim(iname)
            strMsg = "性别：" + Trim(isex)
            '读取性别
'            ccmbSex.Text = Left(Trim(isex),1)
            If Left(Trim(isex), 1) = "男" Then
                ccmbSex.ListIndex = 0
            Else
                ccmbSex.ListIndex = 1
            End If
            ctxtAge.Text = Year(Now()) - Left(Trim(birthday), 4)
            ciptBase.Box1("身份证号").Text = Trim(code)
            Set cctlCatchPhoto.Photo = LoadPicture(App.Path + "\photo.bmp")
        Else
           sffuncMsg "读卡不成功！请重新开始读卡。"
        End If
    Else
        sffuncMsg "未找到卡!请将卡远离后再置于读卡器上。"
       m读卡器状态 = False
    End If
...
errHandler:
    ...
    If Err <> 0 Then
        sfsub错误处理 "体检界面部件", "FrmRegister", "sub读卡信息", Err.Number, Err.Description, True
    End If
...
'    Dim i, j As Long                '定义循环变量
'    Dim sMsg As String              '定义消息返回字串
'    Dim ret As Long                 '定义状态参数变量
'
'    '初始化提示变量
'     cstbMain.Panels(2).Text = ""
'     m读卡器状态 = False
'
'     cstbMain.Panels(2).Text = "身份证读卡器，初始化失败！"
'
'
'    '分别从1001-1017个端口，搜寻设备端口
'    For j = 1001 To 1017
'        If Syn_OpenPort(j) = 0 Then
'            If Syn_GetSAMStatus(j, 0) = 0 Then
'                iPort = j
'                Syn_ClosePort (j)
'                sMsg = "读卡器连接在" + Str(j) + "USB端口上"
'
'                 cstbMain.Panels(2).Text = "身份证读卡器，初始化成功！"
'
'                 m读卡器状态 = True
'                GoTo FIND
'            End If
...
errHandler:
    ...
    If Err <> 0 Then
        sfsub错误处理 "体检界面部件", "FrmRegister", "pCardIdInit", Err.Number, Err.Description, True
    End If
